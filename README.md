...
OpenJFX is the open-source project that develops JavaFX. This project builds [Snap packages](https://snapcraft.io/openjfx) of OpenJFX directly from its [source repository](https://github.com/openjdk/jfx) on GitHub. These packages, together with OpenJDK 25 or later, provide everything you need to develop a JavaFX application on Linux, including all of the latest JAR files, native libraries, JMOD archives, API documentation, and source code of JavaFX.
...
Use these variables in arguments to the `java`, `javac`, `javadoc`, and `jlink` tools of the JDK when you build your JavaFX application. For more information, see the [Usage](#usage) section later.

## Repository

The branches of this repository publish the JavaFX general-availability release (GA) and early-access builds (EA) for the hardware platforms listed below. The table shows the Debian architecture, machine hardware name, and Java architecture of each build:
...
| 2026-07-16 | Rampdown Phase One       | 26 | ←  | 27 | 28 |
| 2026-08-06 | Rampdown Phase Two       | 26 | ←  | 27 | 28 |
| 2026-08-27 | Release Candidate Freeze | 26 | 27 | ←  | 28 |
...
| ------------ | ------------------- | ------------------- | ---------------------- |
| candidate    | [snapcraft.yaml][1] | [openjdk/jfx26u][4] | [openjfx-candidate][7] |
| beta         | [snapcraft.yaml][2] | [openjdk/jfx27u][5] | [openjfx-beta][8]      |
...
[4]: https://github.com/openjdk/jfx26u/tags
[5]: https://github.com/openjdk/jfx27u/tags
...
For example, I'll demonstrate how I verify the OpenJFX Snap package installed on my system at the time of this writing. The `snap info` command shows that I installed OpenJFX version 26.0.2+3 with revision 1789, the revision for the *amd64* architecture:

```console
$ snap info openjfx
...
installed:          26.0.2+3            (1789) 63.1MB -
```
...
image-info:
  build-request-id: lp-117229458
  build-request-timestamp: '2026-08-07T03:26:54Z'
  build_url: https://launchpad.net/~jgneff/openjfx-snap/+snap/openjfx-candidate/+build/3238176
```

The `build_url` in the manifest is a link to the [page on Launchpad](https://launchpad.net/~jgneff/openjfx-snap/+snap/openjfx-candidate/+build/3238176) with the package's **Build status** and **Store status**. The store status shows that Launchpad uploaded revision 1789 to the Snap Store, which matches the revision installed on my system. The build status shows a link to the log file with the label [buildlog](https://launchpad.net/~jgneff/openjfx-snap/+snap/openjfx-candidate/+build/3238176/+files/buildlog_snap_ubuntu_jammy_amd64_openjfx-candidate_BUILDING.txt.gz).
...
Created snap package openjfx_26.0.2+3_amd64.snap
...
a6ea8a150433...f97c67a797d8  openjfx_26.0.2+3_amd64.snap
Revoking proxy token...
[Fri Aug  7 03:38:16 2026]
```

The command below prints the checksum of the package installed on my system:

```console
$ sudo sha512sum /var/lib/snapd/snaps/openjfx_1789.snap
a6ea8a150433...f97c67a797d8  /var/lib/snapd/snaps/openjfx_1789.snap
```
...
```console
$ openjfx
/var/snap/openjfx/1789/openjfx.env
```
...
export JAVAFX_LIB=/snap/openjfx/1789/sdk/lib
export JAVAFX_MOD=/snap/openjfx/1789/sdk/jmods
```
...
JAVAFX_LIB=/snap/openjfx/1789/sdk/lib
JAVAFX_MOD=/snap/openjfx/1789/sdk/jmods
```
...
Once defined, you can use these variables in arguments to the `java`, `javac`, `javadoc`, and `jlink` tools of the JDK.
